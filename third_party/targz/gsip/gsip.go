...
func (r *Reader) acquireReader(off int64) (*gzip.Reader, error) {
	...
	// SectionReader's third arg is length, not absolute end — bound by what
	// remains of the blob from this checkpoint onward, otherwise downstream
	// ReadAt requests can ask for byte ranges past the blob's end.
...
func (r *Reader) ReadAt(p []byte, off int64) (int, error) {
	...
	if err != nil {
		...
		// the partial bytes plus io.EOF, not (0, io.ErrUnexpectedEOF).
